              ...
              ->arrayPrototype()
                ...
                ->children()
                  ...
                  ->scalarNode('reset_password_route_name')
                    ->isRequired()
                    ->info('Route name used as fallback for password reset (required). When redirect_on_expiry is enabled, URLs are generated with this name unless reset_password_route_pattern resolves another name.')
                  ->end()
                  ->scalarNode('reset_password_route_pattern')
                    ->defaultNull()
                    ->info('Optional pattern (glob with * and ?, or PCRE wrapped in the same delimiter e.g. ~^app_reset~). When set, the bundle picks the first registered route name matching the pattern in alphabetical order; if none match, reset_password_route_name is used.')
                  ->end()
                  ->arrayNode('notified_routes')
                    ->scalarPrototype()->end()
                    ->info('Route names or patterns where expiry is enforced (same matching rules as excluded_notified_routes). Empty means no notified routes.')
                  ->end()
                  ->arrayNode('excluded_notified_routes')
                    ->scalarPrototype()->end()
                    ->info('Route names or patterns excluded from expiry checks (literals, globs, or delimited PCRE). If the current route matches any entry, expiry handling is skipped.')
                  ->end()
                  ->booleanNode('detect_password_extensions')
                    ->defaultFalse()
                    ->info('If true, detects when a new password is an extension of an old password (e.g., "password123" is an extension of "password"). This helps prevent users from simply adding numbers or characters to their old passwords.')
                  ->end()
                  ->integerNode('extension_min_length')
                    ->defaultValue(4)
                    ->info('Minimum length of the base password to consider for extension detection. Only passwords longer than this value will be checked for extensions. Default is 4.')
                  ->end()
                ->end()
              ->end()
            ->end()
