 ...
 *     entities: array<string, array{ // Default: []
 *         password_field?: scalar|null, // The name of the password field in the entity. This field will be monitored for changes to track password history. // Default: "password"
 *         password_history_field?: scalar|null, // The name of the password history collection field in the entity. This should be a OneToMany or ManyToMany relationship to a PasswordHistoryInterface entity. // Default: "passwordHistory"
 *         passwords_to_remember?: int, // The maximum number of previous passwords to keep in history. When this limit is exceeded, the oldest passwords are automatically removed. // Default: 3
 *         expiry_days?: int, // Number of days after which a password expires. After this period, users will be notified or redirected to change their password. // Default: 90
 ...
 *         lock_route?: scalar|null, // (Deprecated) Route to redirect when password is expired. Use redirect_on_expiry and reset_password_route_name instead.
 *         redirect_on_expiry?: bool, // If true, automatically redirects users to the reset_password_route_name when their password expires. If false, only shows a flash message without redirecting. // Default: false
 ...
 *             type?: scalar|null, // Flash message type. Common values: "error", "warning", "info", "success". This determines the CSS class and styling of the flash message. // Default: "error"
 *         },
 *     },
 *     log_level?: scalar|null, // Logging level for password policy events. Valid values: "debug", "info", "notice", "warning", "error". All password policy events (expiry detection, password changes, reuse attempts) will be logged at this level. // Default: "info"
 *     enable_logging?: bool, // Enable or disable logging for password policy events. When enabled, important events like password expiry, password changes, and reuse attempts will be logged using Symfony Logger. // Default: true
 *     enable_cache?: bool, // Enable caching for password expiry checks. When enabled, expiry status is cached per user to improve performance. Cache is automatically invalidated when password changes. // Default: false
 *     cache_ttl?: scalar|null, // Cache time-to-live in seconds. Default is 3600 (1 hour). Only used when enable_cache is true. // Default: 3600
 * }
