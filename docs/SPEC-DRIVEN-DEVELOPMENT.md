...
2. **Product behavior** — what **PasswordPolicyBundle** guarantees to applications that integrate it (see [`USAGE.md`](USAGE.md), [`CONFIGURATION.md`](CONFIGURATION.md), [`INSTALLATION.md`](INSTALLATION.md)). **PHPUnit** and **PHPStan** enforce contracts in CI where applicable.
...
| ID | Story |
| --- | --- |
| US-01 | **As a** security officer, **I want** password history and expiry rules **so that** reused or stale passwords are rejected. |
| US-02 | **As an** integrator, **I want** validators and form integration **so that** policy checks run at registration/password change. |
| US-03 | **As an** integrator, **I want** per-entity expiry routes, history limits, and extension detection **so that** organizational policy varies by user type without custom listeners. |
| US-04 | **As a** maintainer, **I want** PHPUnit coverage on validators and entities **so that** edge cases (history hash, expiry) stay correct. |
...
**Goal:** Symfony bundle for password history (reuse prevention), password expiry on configured routes, Doctrine-backed history tracking, and Symfony Validator integration — not password complexity (see PasswordStrengthBundle).
